...
| Not built | Notes |
|---|---|
| Alert delivery | `student/operations.alerts()` returns what has crossed the threshold; nothing emails or webhooks it. Where an alert goes is a deployment decision; no server-side delivery channel exists (see *Reminder delivery from the SERVER* below). |
| A hard spend ceiling under concurrency | **2026-09-24.** The ceiling IS wired: 50 generation model calls per account per rolling 24 hours (a starting value for testing, not a researched limit; `QUINTEK_GENERATION_CALLS_PER_DAY`). Not built: a guarantee under parallel requests — the check and the write are separate statements, so simultaneous requests can overshoot by the number in flight. Also not ceilinged, by decision: concept extraction at ingestion and validator calls. |
...
| Reminder delivery from the SERVER | **2026-09-24 (ADR-031).** Reminders are delivered by the Android app on the phone (AlarmManager + a notification, label verbatim), synced each time the learner opens Reminders. Not built, deliberately: server-side delivery. `ReminderService.fire` still stops at an unset sender and `benchmark.cli notify` is scheduled by nothing — **do not schedule it without a sender: it marks every due reminder failed.** FCM is the recorded route for multi-device or guaranteed delivery. A browser delivers nothing and says so. The phone does not report back, so the server's own status for a reminder stays `pending`; the screen shows the phone's record instead. |
